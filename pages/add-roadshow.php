...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($location) || empty($event_date)) {
        ...
        try {
            // Check if the image_path column exists in the roadshow table
            $columnExists = false;
            $checkStmt = $pdo->query("SHOW COLUMNS FROM roadshow LIKE 'image_path'");
            $columnExists = ($checkStmt->rowCount() > 0);
            
            if (!$columnExists) {
                // Add the column if it doesn't exist
                $pdo->exec("ALTER TABLE roadshow ADD COLUMN image_path VARCHAR(255) DEFAULT NULL");
                error_log("Added image_path column to roadshow table");
            }
            
            // Insert into database
            $stmt = $pdo->prepare("INSERT INTO roadshow (title, description, video_url, location, event_date, is_active, image_path, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$title, $description, $video_url, $location, $event_date, $is_active, $image_path]);
            
            $_SESSION['message'] = "Roadshow added successfully.";
            ...
            $_SESSION['message_type'] = "danger";
            error_log("Database error when adding roadshow: " . $e->getMessage());
        }
    }
}
?>
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">Add New Roadshow</h1>
        ...
        <div class="card-body">
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-1"></i> Save Roadshow
                </button>
            </form>
        </div>
    </div>
