...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update database
    $stmt = $pdo->prepare("UPDATE slider_images SET title_en = ?, title_kh = ?, description_en = ?, description_kh = ?, image_url = ?, button_text_en = ?, button_text_kh = ?, button_url = ?, active = ? WHERE id = ?");
    $result = $stmt->execute([$title_en, $title_kh, $description_en, $description_kh, $image_url, $button_text_en, $button_text_kh, $button_url, $active, $id]);
    ...
    if ($result) {
        ...
        $_SESSION['message'] = "Failed to update slider.";
        $_SESSION['message_type'] = "danger";
    }
}
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800 <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
            <?= $lang === 'en' ? 'Edit Slider' : 'កែប្រែស្លាយ' ?>
        </h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="index.php?page=edit-slider&id=<?= $id ?>" method="POST" enctype="multipart/form-data">
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="title_en" class="form-label <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                            <?= $lang === 'en' ? 'Title (English)' : 'ចំណងជើង (អង់គ្លេស)' ?> <span class="text-danger">*</span>
                        </label>
                        <input type="text" class="form-control" id="title_en" name="title_en" value="<?= htmlspecialchars($slider['title_en']) ?>" required>
                    </div>
                    <div class="col-md-6">
                        <label for="title_kh" class="form-label <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                            <?= $lang === 'en' ? 'Title (Khmer)' : 'ចំណងជើង (ខ្មែរ)' ?> <span class="text-danger">*</span>
                        </label>
                        <input type="text" class="form-control" id="title_kh" name="title_kh" value="<?= htmlspecialchars($slider['title_kh']) ?>" required>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <textarea class="form-control" id="description_en" name="description_en" rows="3"><?= htmlspecialchars($slider['description_en']) ?></textarea>
                    </div>
                    <div class="col-md-6">
                        <label for="description_kh" class="form-label <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                            <?= $lang === 'en' ? 'Description (Khmer)' : 'ការពិពណ៌នា (ខ្មែរ)' ?>
                        </label>
                        <textarea class="form-control" id="description_kh" name="description_kh" rows="3"><?= htmlspecialchars($slider['description_kh']) ?></textarea>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="button_text_en" name="button_text_en" value="<?= htmlspecialchars($slider['button_text_en']) ?>">
                    </div>
                    <div class="col-md-6">
                        <label for="button_text_kh" class="form-label <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                            <?= $lang === 'en' ? 'Button Text (Khmer)' : 'អត្ថបទប៊ូតុង (ខ្មែរ)' ?>
                        </label>
                        <input type="text" class="form-control" id="button_text_kh" name="button_text_kh" value="<?= htmlspecialchars($slider['button_text_kh']) ?>">
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="button_url" name="button_url" value="<?= htmlspecialchars($slider['button_url']) ?>">
                </div>
                
                <div class="mb-3">
                    <label for="image" class="form-label <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                        <?= $lang === 'en' ? 'Slider Image' : 'រូបភាពស្លាយ' ?>
                    </label>
                    <input type="file" class="form-control" id="image" name="image" accept="image/*">
                    <small class="text-muted <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                        <?= $lang === 'en' ? 'Recommended size: 1920x600 pixels. Leave empty to keep current image.' : 'ទំហំដែលណែនាំ៖ 1920x600 ភីកសែល។ ទុកទទេដើម្បីរក្សារូបភាពបច្ចុប្បន្ន។' ?>
                    </small>
                    <?php if (!empty($slider['image_url'])): ?>
                        <div class="mt-2">
                            <p class="mb-1 <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                                <?= $lang === 'en' ? 'Current Image:' : 'រូបភាពបច្ចុប្បន្ន៖' ?>
                            </p>
                            <img src="<?= $slider['image_url'] ?>" alt="Current Slider Image" class="img-thumbnail" style="max-height: 200px;">
                        </div>
                    <?php endif; ?>
                </div>
                
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="active" name="active" <?= $slider['active'] ? 'checked' : '' ?>>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary <?= $lang === 'kh' ? 'khmer-text' : '' ?>">
                        <?= $lang === 'en' ? 'Update Slider' : 'ធ្វើបច្ចុប្បន្នភាពស្លាយ' ?>
                    </button>
                </div>
            </form>
        </div>
